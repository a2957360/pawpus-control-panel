...
export default function FoodForm(props) {
		...
		form.validateFields().then(values => {
			setTimeout(() => {
				...
				if (mode === ADD) {
					...
					values['componentContent'].map((item,index)=>{
						console.log(values['componentContent'][index]);
						values['componentContent'][index]['image'] = coverImg[index];
					})
	...
	useEffect(() => {
		if (mode === EDIT && homePageData != null) {
			...
			let image = [];
			item.componentContent.map(item=>{
