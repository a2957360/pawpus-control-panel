			...
			<Row gutter={16}>
				...
				<Col xs={24} sm={24} md={17}>
					<Card title="Basic Info">
						...
						<Form.Item name='endDate' label="服务结束日期" rules={rules.title}>
						...
						<Form.Item name="serviceLanguage" label="服务语言" >
							<Select
								mode="multiple"
								size="default"
						...
						<Row gutter={16}>
							...
							</Col>
							{
								...
								categories.map(item => (
									...
									item.parentCategoryId == categoryId &&
									<Col span={6}>
										<Form.Item name={['serviceSubCategory', item.categoryId]} label={item.categoryName.zh} >
						...
						<Row gutter={16}>
...

							{
								...
								categories.map(item => (
									item.categoryType == 1 &&
									<Col span={6}>
										<Form.Item name={['serviceExtra', item.categoryId]} label={item.categoryName.zh} >
											<Input placeholder="价格 没有该服务请留空" />
										</Form.Item>
									</Col>
								))}
						</Row>
						<Form.Item name="serviceFacility" label="基础设施" >
							<Select
								mode="multiple"
								size="default"
								placeholder="Please select"
						...
						<Row gutter={16}>
							<Col span={12}>
								<Form.Item name='checkinTime' label="入住时间" rules={rules.title}>
									<Input placeholder="checkinTime" />
								</Form.Item>
							</Col>
							<Col span={12}>
								<Form.Item name='checkoutTime' label="退房时间" rules={rules.title}>
									<Input placeholder="checkoutTime" />
								</Form.Item>
							</Col>
						</Row>
						...
						<Form.Item name='servicePostal' label="邮编" rules={rules.title}>
							<Input placeholder="servicePostal" />
						</Form.Item>
						<Form.Item name="serviceHourseType" label="居住类型" >
							<Select
								placeholder="Please select"
								style={{ width: '100%' }}
							>
								<Option key={0} value={0}>公寓</Option>
								<Option key={1} value={1}>独栋别墅</Option>
								<Option key={2} value={2}>联排别墅</Option>
								<Option key={3} value={3}>宠物机构</Option>
								{/* {
									categories != null &&
									categories.map(item => (
										item.categoryType == 6 &&
						...
						<Form.Item name='serviceDescription' label="详细介绍" rules={rules.title}>
							<TextArea rows={4} />
						</Form.Item>
						<Form.Item name='serviceRequirement' label="入住须知" rules={rules.title}>
							<TextArea rows={4} />
						</Form.Item>
					</Card>
				</Col>
